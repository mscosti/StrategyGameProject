...
public class DeltaStrategyGameController implements StrategyGameController {
	 ...
	 */
	public DeltaStrategyGameController(List<PieceLocationDescriptor> redConfigurationList, 
			...
			List<PieceLocationDescriptor> chokePointConfigurationList) throws StrategyException
	{
		...
		battleRules = new DeltaBattleRules();
		board = new Board(10, 10);
		history = new HistoryLogger();

		final StrategyValidConfiguration deltaValidator = new DeltaValidConfiguration(
				redConfigurationList, blueConfigurationList, board);

		if (!deltaValidator.isCorrectPieceInput(BLUE) ||
				!deltaValidator.isCorrectPieceInput(RED))
		{
			throw new StrategyException("Incorrect Piece Input");
		}

		final int blueLocationCheck = deltaValidator.isCorrectLocationInput(BLUE, 4);
		final int redLocationCheck = deltaValidator.isCorrectLocationInput(RED, 4);
	...
	public MoveResult move(PieceType piece, Location from, Location to) throws StrategyException 
	{
		...
		final StrategyMoveRules deltaMoveRules = new DeltaMoveRules(board, fromPiece, to, history, thisMove, pieceHandler);
		deltaMoveRules.checkValidGameState(gameStarted, gameOver);
		deltaMoveRules.checkValidTurnOrder(history.getTurnCount());
		deltaMoveRules.checkValidMove();
